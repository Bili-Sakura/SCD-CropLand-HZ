...
from ChangeMamba.changedetection.datasets.make_data_loader import ChangeDetectionDatset, make_data_loader, DamageAssessmentDatset
from ChangeMamba.changedetection.utils_func.metrics import Evaluator
from ChangeMamba.changedetection.models.ChangeMambaBDA import ChangeMambaBDA
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            if (itera + 1) % 10 == 0:
                ...
                log_metrics({"train/loc_loss": float((ce_loss_loc + lovasz_loss_loc).item()), "train/clf_loss": float((ce_loss_clf + lovasz_loss_clf).item()), "train/loss": float(final_loss.item())}, step=itera + 1)
                if (itera + 1) % 750 == 0:
                    self.deep_model.eval()
                    loc_f1_score, harmonic_mean_f1, oaf1, damage_f1_score = self.validation()
                    log_metrics({"val/loc_f1": loc_f1_score, "val/harmonic_mean_f1": harmonic_mean_f1, "val/oaf1": oaf1}, step=itera + 1)
                    if oaf1 > best_kc:
