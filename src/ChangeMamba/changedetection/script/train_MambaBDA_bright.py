...
from ChangeMamba.changedetection.datasets.make_data_loader import make_data_loader, MultimodalDamageAssessmentDatset
from ChangeMamba.changedetection.utils_func.metrics import Evaluator
from ChangeMamba.changedetection.models.ChangeMambaMMBDA import ChangeMambaMMBDA
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            if (itera + 1) % 10 == 0:
                ...
                log_metrics({"train/clf_loss": float((ce_loss_clf + lovasz_loss_clf).item()), "train/loss": float(final_loss.item())}, step=itera + 1)
                if (itera + 1) % 500 == 0:
                    ...
                    log_metrics({
                        "val/loc_f1": loc_f1_score_val, "val/harmonic_mean_f1": harmonic_mean_f1_val,
                        "val/OA": final_OA_val, "val/mIoU": mIoU_val,
                        "test/loc_f1": loc_f1_score_test, "test/harmonic_mean_f1": harmonic_mean_f1_test,
                        "test/OA": final_OA_test, "test/mIoU": mIoU_test
                    }, step=itera + 1)
