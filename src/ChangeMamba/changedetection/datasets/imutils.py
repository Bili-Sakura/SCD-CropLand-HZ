...
def random_fliplr_mcd(pre_img, post_img, label_cd, label_1, label_2):
    if random.random() > 0.5:
        label_cd = np.ascontiguousarray(np.fliplr(label_cd))
        label_1 = np.ascontiguousarray(np.fliplr(label_1))
        label_2 = np.ascontiguousarray(np.fliplr(label_2))
        pre_img = np.ascontiguousarray(np.fliplr(pre_img))
        post_img = np.ascontiguousarray(np.fliplr(post_img))
